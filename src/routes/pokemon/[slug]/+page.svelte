<script lang="ts">
    ...
    function getFlavorText() {
		for(let i = 0; i < data.poke.specie.flavor_text_entries.length; i++){
			if(data.poke.specie.flavor_text_entries[i].language.name === "en") return data.poke.specie.flavor_text_entries[i].flavor_text;
		}
	}
</script>
...
<div class="container m-auto">
    {#if data.poke.mon}
    <div
        class="flex flex-row justify-between items-center gap-12 m-8"
        id="header-data"
    >
    ...
    <div class="flex flex-row">
        <img
            ...
            src={
                data.poke.mon.sprites.front_default
    ...
    {:else}
        <img class="w-32 h-32 border-2 border-red-600" src={pokeball} alt="pokeloading">
    {/if}
